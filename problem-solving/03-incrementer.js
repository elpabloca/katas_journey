...
its position in the array: the first digit will be incremented by 1, 
...
function incrementerByIndex(arr) {
    const newArray = []

    for(let i = 0; i < arr.length; i++) {
        const sum = lastDigit(arr[i] + i + 1)
        newArray.push(sum)
    }
    return newArray
}
...
const numbers = [11,15,2,7, 20]
console.log(incrementerByIndex(numbers))
console.log(incrementer(numbers))